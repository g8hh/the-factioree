...
addLayer("f", {
	...
	tabFormat: {
		...
		"Upgrades": {
			content: ["main-display", "prestige-button", "resource-display", ["raw-html", "<br><h2>Furnace Upgrades</h2><br>"],
...
addLayer("e", {
	...
	tabFormat: {
		"Main": {
			content: ["main-display", "prestige-button", "resource-display", ["raw-html", "<br>"], "milestones", ["raw-html", "<br>"], "buyables", ["raw-html", "<br>"], "upgrades"]
		},
		"Oil": {
...
addLayer("m", {
	...
	tabFormat: {
		"Main": {
			content: ["main-display", "prestige-button", "resource-display", ["upgrade", 15], ["raw-html", "<br>Reach 80 manufacturers to unlock the next layer.<br>"], "milestones", ["raw-html", "<br>"], ["row", [["upgrade", 11], ["upgrade", 12], ["upgrade", 13], ["upgrade", 14]]]]
		},
		"Factories": {
...
addLayer("r", {
	...
	prestigeNotify(testValue=tmp.r.resetGain) {
		...
		return testValue.gte(player.r.points.div(20))
	}
})
...
addLayer("mo", {
	...
	buyables: {
		...
		11: {
			...
			effect() {
				if (player.l.upgrades.includes(11)) return new Decimal(0);
		...
		13: {
			...
			unlocked() {
				return player.ps.unlocked&&!player.l.upgrades.includes(11)
			}
		},
		31: {
			...
			display() {
				...
				Clean ocean/s: ${format(tmp.mo.buyables[31].effect)}`
			},
			buy() {
				if (this.canAfford()) {
	...
	tabFormat: {
		...
		"Pollutions": {
			content: ["main-display", "prestige-button", "resource-display", ["raw-html", _=> {
				return player.l.upgrades.includes(11)?`<br>Each purchased AntiPollution gives 0.1 happiness.<br>
				You have filler text filler currency, boosting filler currency by filler effect
				<br><br><h2><s>Pollutions</s> &nbsp; AntiPollutions</h2><br><br>You have ${format(player.l.crowdfund)} crowdfunding.`
				:`<br>You have ${format(player.mo.waste)} waste, boosting plastic gain by ${format(tmp.mo.wasteEffect.plasticBoost)} and making manufacturers cheaper by /${format(tmp.mo.wasteEffect.manuCheap)}.<br><br>
				You have ${format(player.mo.burningWaste)} burning waste, boosting waste gain by ${format(tmp.mo.wasteEffect.wasteBoost)}.<br>
				${player.ps.unlocked?`You have ${format(player.mo.greenhouse)} greenhouse gas, boosting burning waste gain by ${format(tmp.mo.wasteEffect.burningWasteBoost)},
	...
	tabFormat: {
		...
		"Guide": {
			content: [["raw-html", _=> {
				...
				People are no more than robots under your rule.<br>
				They would very much like to rebel, but you enforce a very strict measure that forces people to work at least 25 hours a day in the mines.<br>
				Do whatever you want. Your care for humans is insignificant. You might as well profit off of peoples' uncontent.`
			}]],
			unlocked() {
...
addLayer("l", {
	...
	milestones: {
		0: {
			...
			effectDescription: "Keep all previous layer milestones.<br>And also researches because they were super timewally.",
	...
	effect() {
		return {
			...
			crowdfundps: player.l.happiness.pow(0.5).mul(0.1)
		}
	},
})
addLayer("d", {
	...
	milestones: {
		0: {
			...
			effectDescription: "Keep all previous layer milestones and challenge completions.<br>And also researches because they were super timewally.",
